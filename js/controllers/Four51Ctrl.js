four51.app.controller('Four51Ctrl', ['$scope', '$route', '$location', '$451', 'User', 'Order', 'Security', 'OrderConfig', 'Category', 'AppConst','XLATService','$modal','$rootScope','SpendingAccount',
function ($scope, $route, $location, $451, User, Order, Security, OrderConfig, Category, AppConst, XLATService, $modal, $rootScope, SpendingAccount) {
	$scope.AppConst = AppConst;
	if (sessionStorage.CrossPageVars)
	    $scope.CrossPageVars = JSON.parse(sessionStorage.getItem('CrossPageVars'));
	else
	    $scope.CrossPageVars = {};
    ...
    $scope.saveCrossPageVars = function(vars) {
        sessionStorage.setItem('CrossPageVars', JSON.stringify(vars));
    }
    
    $scope.getCrossPageVars = function() {
		$scope.CrossPageVars = JSON.parse(sessionStorage.getItem('CrossPageVars'));
    }
    
    $scope.saveWhatsNew = function(prods) {
        sessionStorage.setItem('whatsNew', JSON.stringify(prods));
    }
    
    $scope.getWhatsNew = function() {
		$scope.whatsNew = JSON.parse(sessionStorage.getItem('whatsNew'));
    }
    
    $scope.checkPath = function () {
        if (typeof $scope.Four51User.currentUser === 'undefined')
            return 'ng-hide';
    }
    
    $scope.hideNav = function () {
        if ($location.path().indexOf('landing') != -1 || $location.path().indexOf('catalog') != -1 || $location.path().indexOf('note') != -1)
            return 'ng-hide';
    }
